...
const isPointDraggable = (point) => {
    if (!point)
        return false;
    if (point.construction_kind === 'intersection')
        return false;
    if (point.construction_kind === 'midpoint')
        return false;
    if (point.construction_kind === 'symmetric')
        return false;
    // Check if point is center of a three-point circle (computed center, not draggable)
    const pointIdx = model.points.indexOf(point);
    if (pointIdx >= 0) {
        const isThreePointCenter = model.circles.some(c => isCircleThroughPoints(c) && c.center === pointIdx);
        if (isThreePointCenter)
            return false;
    }
    return true;
};
const isDefiningPointOfLine = (pointIdx, lineIdx) => {
...
function draw() {
    ...
    model.points.forEach((p, idx) => {
        ...
        if ((highlightPoint ||
            ...
            (selectedCircleIndex !== null && selectionVertices && (circleHasDefiningPoint(model.circles[selectedCircleIndex], idx) ||
    ...
    if (mode === 'multiselect') {
        ...
        multiSelectedInkStrokes.forEach(idx => {
            ...
            const margin = screenUnits(8);
            ctx.beginPath();
            ctx.rect(bounds.minX - margin, bounds.minY - margin, bounds.maxX - bounds.minX + margin * 2, bounds.maxY - bounds.minY + margin * 2);
            ctx.stroke();
        });
        ctx.setLineDash([]);
        ctx.restore();
...
function initAppearanceTab() {
    ...
    function loadThemeValues() {
        ...
        if (themeFontSizeValue)
            themeFontSizeValue.textContent = `${current.fontSize || base.fontSize} px`;
        if (themeHighlightWidthValue)
            themeHighlightWidthValue.textContent = `${current.highlightWidth || base.highlightWidth} px`;
                    ...
                    if (centerRadiusCircles.length) {
                        ...
                        if (selectedPointIndex !== null) {
                            ...
                            linesWithPoint.forEach((lineIdx) => {
                                const line = model.lines[lineIdx];
                                if (!line || !line.defining_points.includes(idx))
                                    return;
                                const definingPoints = line.defining_points.map(i => model.points[i]).filter(Boolean);
                            ...
                            if (definingPoints.length >= 2) {
                                ...
                                if (lineLength > 0) {
                                    mainLine.points.forEach((pIdx) => {
                                        ...
                                        if (mainLine.defining_points.includes(pIdx))
                                            return;
                                        const pt = model.points[pIdx];
                                        if (!pt)
                                            return;
                        ...
                        if (origin && end) {
                            ...
                            if (len > 0) {
                                l.points.forEach((pIdx, i) => {
                                    if (l.defining_points.includes(pIdx))
                                        return;
                                    const t = fractions[i];
                                    const newPos = {
    ...
    hideBtn?.addEventListener('click', () => {
        ...
        if (selectedInkStrokeIndex !== null) {
            ...
            if (selectionVertices) {
                const circle = model.circles[selectedCircleIndex];
                const pointsToToggle = new Set();
                if (circle.center !== null)
                    pointsToToggle.add(circle.center);
                if (circle.radius_point !== null)
                    pointsToToggle.add(circle.radius_point);
        ...
        if (selectedInkStrokeIndex !== null) {
            ...
            if (circle) {
                if (selectionVertices) {
                    ...
                    if (circle.center !== null)
                        pointsToDelete.add(circle.center);
                    if (circle.radius_point !== null)
                        pointsToDelete.add(circle.radius_point);
                    circle.defining_parents.forEach(pid => {
                        const idx = model.points.findIndex(p => p.id === pid);
                        if (idx !== -1)
...
function measureFormattedText(ctx, text) {
...
function remapAngles(lineRemap) {
    model.angles = model.angles.filter((ang) => {
