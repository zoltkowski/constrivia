...
- `src/core/convert.ts`: Conversions between legacy data and runtime-compatible structures.
- `src/core/angleUtils.ts`: Angle-specific helpers used by the engine and UI.
...
- `src/core/styleState.ts`: Core style state helpers for runtime-aware rendering and UI.
...
- `src/model.ts`: Base model types and model container utilities.
